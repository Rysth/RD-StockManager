...
interface UserState {
  ...
  batchToggleConfirmation: (userIds: number[], isConfirmed: boolean) => Promise<void>;
...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  batchDeleteUsers: async (ids: number[]) => {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Error al eliminar usuarios";
      set({ error: message, isLoading: false });
      throw new Error(message);
    }
  },

  batchToggleConfirmation: async (userIds: number[], isConfirmed: boolean) => {
    set({ isLoading: true, error: null });
    try {
      await Promise.all(
        userIds.map((id) =>
      ...
      const message =
        error.response?.data?.message || "Error al actualizar confirmaciones";
      set({ error: message, isLoading: false });
      throw new Error(message);
    }
  },
